...
export default function AdminSeasonalPage() {
  ...
  const [records, setRecords] = useState<SeasonalItemRecord[]>([]);
  const [products, setProducts] = useState<SeasonalItemProduct[]>([]);
  ...
  const load = async () => {
    setBusy(true);
    try {
      const res = await fetch("/api/admin/seasonal", { headers: adminHeaders() });
      const data = await res.json();
      if (!res.ok) throw new Error(data?.error || "Failed to load Seasonal.");
  ...
  const editRow = (record: SeasonalItemRecord) => {
    setForm({
      sku: record.sku,
      note: record.note || "",
  ...
  const removeRow = async (targetSku: string) => {
    ...
    if (!confirm(`Remove ${sku} from Seasonal?`)) return;

    setBusy(true);
    try {
      const res = await fetch(`/api/admin/seasonal?sku=${encodeURIComponent(sku)}`, {
        method: "DELETE",
  ...
  return (
    <AdminPage
      active="seasonal"
      title="Seasonal"
      subtitle="Curate SKUs shown on the customer Seasonal tab. Add one by one or paste a list."
      actions={
        <>
          <BtnSecondary
            onClick={() => {
      ...
      <div style={splitLayout} className="admin-catalog-split admin-split">
        <Panel title={`List (${filtered.length})`}>
          ...
          <div className="admin-split-list">
            {filtered.length === 0 ? (
              <EmptyState title="No SKUs yet" detail="Add seasonal SKUs on the right, or paste a bulk list below." />
            ) : (
              filtered.map((r) => {
                const product = productMap.get(r.sku.toUpperCase());
                return (
