...
import { inputStyle } from "@/app/admin/_components/admin-styles";
import { useCompAuth } from "./useCompAuth";

type AccountHit = { accountNo: string; storeName: string };

export function CompAccountAutocomplete({
  value,
  onChange,
  onPick,
    ...
    async (query: string) => {
      ...
      try {
        const res = await fetch(
          `/api/comp/account-suggest?q=${encodeURIComponent(query.trim())}&limit=12`,
          { headers: compHeaders() }
        );
